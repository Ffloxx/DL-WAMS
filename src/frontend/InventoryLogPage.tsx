    ...
    <main className="flex-1 flex flex-col p-6 space-y-5 overflow-y-auto">
      ...
      <header className="flex justify-between items-start shrink-0">
        <div>
          ...
          <h1 className="text-3xl font-bold tracking-wider text-slate-100 uppercase">Inventory Log</h1>
          <p className="text-xs text-slate-400 mt-2 max-w-lg leading-relaxed">
            Centralized stock synchronization for Site_Alpha logistics cluster. Automated threshold monitoring engaged.
          </p>
        </div>
        <div className="flex items-center gap-3">
          <button onClick={handleExportCSV} className="flex items-center gap-2 px-4 py-2.5 bg-slate-800/50 border border-slate-700/50 rounded-sm text-[10px] font-bold tracking-widest text-slate-300 uppercase hover:bg-slate-700/50 hover:text-white transition-colors">
            <Download className="w-3.5 h-3.5" /> Export CSV
          </button>
          <button onClick={handleBatchRestock} className="flex items-center gap-2 px-4 py-2.5 bg-slate-800/50 border border-slate-700/50 rounded-sm text-[10px] font-bold tracking-widest text-slate-300 uppercase hover:bg-slate-700/50 hover:text-white transition-colors">
            <RotateCcw className="w-3.5 h-3.5" /> Batch Restock
          </button>
          <button onClick={() => setShowModal(true)} className="flex items-center gap-2 px-4 py-2.5 bg-blue-600/80 border border-blue-500/60 rounded-sm text-[10px] font-bold tracking-widest text-white uppercase hover:bg-blue-500/80 transition-colors shadow-[0_0_12px_rgba(59,130,246,0.3)]">
      ...
      <div className="flex gap-5 flex-1 min-h-0">
        ...
        <div className="w-[280px] shrink-0 flex flex-col gap-4">
          <div className="bg-[#0f1522] border border-slate-800 rounded-sm p-4 flex-1">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-xs font-bold tracking-widest text-slate-200 uppercase">Activity Log</h3>
              ...
              {ACTIVITY_LOG.map((entry, i) => (
                <div key={i} className="flex gap-3">
                  <div className="flex flex-col items-center shrink-0 pt-1">
                    <span className={`w-2.5 h-2.5 rounded-full ${entry.color} shrink-0`} />
                    {i < ACTIVITY_LOG.length - 1 && <div className="w-px flex-1 bg-slate-800 mt-1" />}
                  </div>
                  <div className="pb-2">
                    <div className="text-[9px] text-slate-500 font-mono mb-1">{entry.time} // {entry.auth}</div>
                    <p className="text-[11px] text-slate-300 leading-relaxed">
                      {entry.title && <span className="font-bold text-slate-200">{entry.title} </span>}
                      {entry.content}
                    </p>
                  </div>
                </div>
          ...
          {/* Thermal Label Engine */}
          <div className="bg-[#0f1522] border border-slate-800 rounded-sm p-4">
            <div className="flex items-center gap-2 mb-3">
              <Package className="w-4 h-4 text-slate-400" />
              <h3 className="text-[10px] font-bold tracking-widest text-slate-200 uppercase">Thermal Label Engine</h3>
            </div>
            <p className="text-[10px] text-slate-500 leading-relaxed mb-3">
              {labelsPending} labels pending for queue 01-ALPHA. Ready for physical output.
            </p>
            <button onClick={handlePrintLabels}
              className="w-full flex items-center justify-center gap-2 py-2.5 bg-slate-800/60 border border-slate-700 rounded-sm text-[10px] font-bold tracking-widest text-slate-300 uppercase hover:bg-slate-700/50 hover:text-white transition-colors">
