  ...
  return (
    <main className="flex-1 flex flex-col overflow-hidden">
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        {/* Right Panel */}
        <div className="w-[320px] shrink-0 border-l border-slate-800 bg-[#0a0f18] flex flex-col p-4 gap-4 overflow-y-auto">
          {/* Camera Feed */}
          <div>
            <div className="flex items-center gap-2 mb-2">
              <Video className="w-3.5 h-3.5 text-slate-500" />
              <span className="text-[9px] font-bold tracking-widest text-slate-500 uppercase">Live Feed</span>
            </div>
            <CameraFeed />
          </div>
          ...
          <div className="bg-[#0f1522] border border-slate-800 rounded-sm p-4">
            <div className="flex justify-between items-center mb-3">
              <div>
                <h3 className="text-[10px] font-bold tracking-widest text-slate-400 uppercase">Loyalty Node</h3>
                <p className="text-[9px] text-slate-500 mt-0.5">Tier: {LOYALTY_MEMBER.tier}</p>
              </div>
              <button onClick={() => setLoyaltyLinked(!loyaltyLinked)} className={`transition-colors ${loyaltyLinked ? 'text-blue-400' : 'text-slate-600'}`}>
                <BadgeCheck className="w-5 h-5" />
              </button>
            </div>
            {loyaltyLinked && (
              <>
                <div className="flex items-center gap-3 mb-3 p-2.5 bg-slate-900/60 border border-slate-800 rounded-sm">
                  <div className="w-9 h-9 bg-blue-600/20 border border-blue-500/30 rounded-full flex items-center justify-center text-[10px] font-bold text-blue-400 tracking-wider shrink-0">
                    {LOYALTY_MEMBER.initials}
                  </div>
                  <div>
                    <div className="text-xs font-bold text-slate-200 tracking-wider">{LOYALTY_MEMBER.name}</div>
                    <div className="text-[9px] text-slate-500 font-mono">UID: {LOYALTY_MEMBER.uid}</div>
                  </div>
                </div>
                <div className="grid grid-cols-2 gap-2">
                  <div className="bg-slate-900/60 border border-slate-800 rounded-sm p-2.5 text-center">
                    <div className="text-[8px] text-slate-500 font-bold tracking-widest uppercase mb-1">Accrued Points</div>
                    <div className="text-sm font-bold text-slate-200 font-mono">{LOYALTY_MEMBER.points.toLocaleString()} pts</div>
                  </div>
                  <div className="bg-slate-900/60 border border-slate-800 rounded-sm p-2.5 text-center">
                    <div className="text-[8px] text-slate-500 font-bold tracking-widest uppercase mb-1">Available Credits</div>
                    <div className="text-sm font-bold text-slate-200 font-mono">${LOYALTY_MEMBER.credits.toFixed(2)}</div>
                  </div>
                </div>
              </>
            )}
          ...
          <button
            ...
            className={`w-full py-4 rounded-sm font-bold tracking-widest text-xs uppercase transition-all duration-300 flex flex-col items-center gap-1 ${
              alarmTriggered
                ? 'bg-red-600 border border-red-500 text-white animate-pulse shadow-[0_0_30px_rgba(239,68,68,0.5)]'
                : 'bg-red-600/15 border border-red-500/40 text-red-400 hover:bg-red-600/25 hover:border-red-400/60'
            }`}
          >
            <ShieldAlert className="w-5 h-5" />
            <span className="text-sm font-bold tracking-widest">{alarmTriggered ? 'ALARM DISPATCHED' : 'SILENT ALARM'}</span>
            <span className="text-[8px] text-red-400/80 tracking-wider">{alarmTriggered ? 'HQ NOTIFIED' : 'DIRECT HQ DISPATCH'}</span>
          </button>
        </div>
      </div>

      {/* Status Bar */}
      <footer className="flex items-center justify-between px-5 py-1.5 border-t border-slate-800 bg-[#0a0f18]/80 shrink-0">
        <span className="text-[8px] text-slate-600 font-mono tracking-wider">AEGIS_POS V2.1 // TERMINAL_NODE_04</span>
        <div className="flex items-center gap-6">
          <span className="text-[8px] text-slate-600 font-mono tracking-wider">ENCRYPTION_STATUS: AES-256</span>
          <span className="text-[8px] text-slate-600 font-mono tracking-wider">LATENCY: 12ms</span>
          <span className="text-[8px] text-slate-600 font-mono tracking-wider">BUFFER: OK</span>
          <span className="text-[8px] text-slate-600 font-mono tracking-wider">{now.toTimeString().slice(0, 8)}</span>
        </div>
      </footer>
    </main>
  );
