...
export const Sidebar = ({ activeTab, onTabChange, onLogout }: SidebarProps) => {
  const navItems: { icon: React.ReactNode; label: TabName }[] = [
        ...
        <nav className="space-y-1">
          {navItems.map((item, index) => (
            <button
              ...
              className={`w-full flex items-center space-x-4 px-8 py-4 text-xs font-semibold tracking-widest uppercase transition-all duration-200 ${
